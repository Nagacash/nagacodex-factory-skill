...
A collection of [agent skills](https://code.claude.com/docs/en/skills) for Claude Code / OpenCode. Each skill is a folder containing a `SKILL.md` with frontmatter (name, description) and instructions that the agent loads on demand when the task matches. The factory orchestrator `naga-factory` ties them together via [`AGENTS.md`](AGENTS.md).
...
## Naga Codex Factory Workflow

[`AGENTS.md`](AGENTS.md) ties the skills together into the Naga Codex Factory four-beat workflow: **Isolate** (`/new-feature`) → **Build** (`/code-structure`) → **Prove** (`/evidence-driven-testing`) → **Ship** (`/before-and-after` + `/greploop`), with `/unslop` applied to everything written for humans along the way. Invoke the factory with `/naga-factory` or any individual beat directly. Drop `AGENTS.md` into a repo alongside the skills and fill in the repo-specific callouts (checks, invariants, environment).

## Installation — Naga Codex Factory Skill

Global factory install (OpenCode + Claude Code):

```bash
# Full factory — available in all projects
cp -r naga-factory ~/.config/opencode/skills/
cp -r naga-factory ~/.claude/skills/
# constituent beats are also available individually
for s in new-feature code-structure evidence-driven-testing before-and-after greploop greploop-apps unslop; do
  cp -r "$s" ~/.config/opencode/skills/
  cp -r "$s" ~/.claude/skills/
done
# or symlink to this repo for live updates:
# ln -s /path/to/factory-skills-main/naga-factory ~/.config/opencode/skills/naga-factory
```

Or clone and copy a single beat:
...
Claude Code / OpenCode picks up the skill automatically and invokes it when a task matches the skill's description. You can also invoke explicitly with `/naga-factory` (full pipeline) or any beat `/new-feature`, `/code-structure`, `/evidence-driven-testing`, `/before-and-after`, `/greploop`, `/unslop`.
